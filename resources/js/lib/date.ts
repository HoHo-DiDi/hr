...
export function toDisplay(date: Date | string | null | undefined, withTime = false):string{
    if(!date) return "-";
    const format = withTime ? DATE_CONFIG.DISPLAY_DATETIME : DATE_CONFIG.DISPLAY_DATE;
    return dayjs(date).format(format);
}

export function toDB(date: Date | string | null | undefined, withTime = false):string{
    if(!date) return "";
